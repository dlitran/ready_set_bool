...
void adder_tester(void)
{
	...
	a = 100329;
	b = 60;
	result = readySetBoole::adder(a, b);
	...
	for (int it = 0; it < 100; it++)
	{
		...
		if (readySetBoole::adder(a, b) != (a + b))
		{
			std::cout << "\033[31mIncorrect\033[0m" << std::endl;
			return ;
		}
...
void multiplier_tester(void)
{
	...
	a = 105;
	b = 6;
	result = readySetBoole::multiplier(a, b);
	std::cout << a << " * " << b  << " = " << result << std::endl;
	print_binary(result);
	if (result == (a * b))
		std::cout <<  "\033[32mCorrect\033[0m" << std::endl;
	...
	for (int it = 0; it < 100; it++)
	{
		...
		b = rand();
		// std::cout << "a: " << a << " b: " << b << std::endl;
		if (readySetBoole::multiplier(a, b) != (a * b))
		{
			std::cout << "\033[31mIncorrect\033[0m" << std::endl;
			return ;
		}
...
void gray_code_tester(void)
{
	...
	print_binary(num);
	gray = readySetBoole::gray_code(num);
	print_binary(gray);
...
void	testFormula(std::string &formula)
{
	...
	statement = readySetBoole::eval_formula(formula);
	if (statement == true)
...
void print_truth_table_tester(void)
{
	...
	readySetBoole::print_truth_table(formula);
...
std::vector<bool>	testEquivalence(std::string formula)
{
	...
	numProposition = readySetBoole::countPropositionsIndex(formula, indexArray);
	permutation = 0;
	while (permutation < (1 << numProposition))
	{
		result.insert(result.end(), readySetBoole::optimized_eval_formula(formula, numProposition,  indexArray, permutation));
		permutation++;
	}
	return (result);
}
...
void powerset_tester(void)
{
	...
	std::vector<int> set = {1, 2, 3, 4, 5, 6};
		std::vector<std::vector<int>> powerSet = readySetBoole::powerset(set);
...
void print_result_set(std::string formula, std::vector<std::vector<int>> sets)
{
	...
	result = readySetBoole::eval_set(formula, sets);
	std::cout << "{ ";
